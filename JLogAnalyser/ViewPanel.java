	...
	public void appendToLogText(String text) {
		...
		synchronized (logView) {
			...
			if (docLen <= (capacity * capCount)) {
				...
				try {
					//logView.getAccessibleContext().getAccessibleEditableText().setTextContents(null);
					logView.getAccessibleContext().getAccessibleEditableText().setTextContents(text);
