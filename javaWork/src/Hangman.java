...
public class Hangman extends java.applet.Applet implements Runnable, MouseListener, KeyListener {
	...
	/*
	 * These offsets refer to the dance images. The dance images are not of the
	 * same size so we need to add these offset in order to make the images
	 * "line" up.
	 */
	private int danceImageOffsets[] = { 8, 0, 0, 8, 18, 21, 27 };

	/*
	 * This represents the sequence to display the dance images in order to make
	 * Duke "dance".
	 */
	private int danceSequence[] = { 3, 4, 5, 6, 6, 5, 6, 6, 5, 4, 3, 2, 1, 0, 0, 1, 2, 2, 1, 0, 0, 1, 2 };

	/*
	 * This is the current sequence number. -1 implies that Duke hasn't begun to
	 * dance.
	 */
	int danceSequenceNum = -1;

	/*
	 * This variable is used to adjust Duke's x-position while he's dancing.
	 */
	int danceX = 0;

	/*
	 * This variable specifies the currently x-direction of Duke's dance.
	 * 1=>right and -1=>left.
	 */
	...
	String wordlist[] = { "abstraction", "ambiguous", "arithmetic", "backslash", "bitmap", "circumstance", "combination",
			"consequently", "consortium", "decrementing", "dependency", "disambiguate", "dynamic", "encapsulation", "equivalent",
			"expression", "facilitate", "fragment", "hexadecimal", "implementation", "indistinguishable", "inheritance",
			"internet", "java", "localization", "microprocessor", "navigation", "optimization", "parameter", "patrick", "pickle",
			"polymorphic", "rigorously", "simultaneously", "specification", "structure", "lexical", "likewise", "management",
			"manipulate", "mathematics", "hotjava", "vertex", "unsigned", "traditional" };
	...
	public void init() {
		...
		resize((maxWordLen + 1) * wordFontMetrics.charWidth('M') + maxWordLen * 3, hangImagesHeight * 2
	...
	public void mousePressed(MouseEvent e) {
		...
		if (secretWordLen > 0 && (secretWordLen == wordLen || wrongLettersCount == maxTries)) {
			newGame();
		} else {
			play(getDocumentBase(), "audio/beep.au");
		}
	...
	/**
	 * Starts a new game. Chooses a new secret word and clears all the buffers
	 */
	public void newGame() {
		int i;
	...
	public void paint(Graphics g) {
		...
		if (secretWordLen > 0) {
			...
			if (wordLen < secretWordLen && wrongLettersCount > 0) {
				...
				g.drawImage(hangImages[wrongLettersCount - 1], baseW, imageH / 3, this);
			}
		}
	...
	void updateDancingDuke(Graphics g) {
		...
		g.clearRect(danceX + baseW, imageH * 2 - danceHeight, danceImageOffsets[danceImageNum] + danceImageWidths[danceImageNum],
				danceHeight);
		...
		if ((danceImageNum < danceImagesLen) && (danceImages[danceImageNum] != null)) {
			g.drawImage(danceImages[danceImageNum], danceX + baseW + danceImageOffsets[danceImageNum], imageH * 2 - danceHeight,
					this);
		}
	}
